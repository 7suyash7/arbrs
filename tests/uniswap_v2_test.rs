...
use arbrs::pool::uniswap_v2::UniswapV2PoolState;
use arbrs::{ArbRsError, TokenLike};
use arbrs::manager::token_manager::TokenManager;
use arbrs::pool::{LiquidityPool, uniswap_v2::UniswapV2Pool};
use std::sync::Arc;
use url::Url;
...
async fn setup_v2_pool() -> (
    Arc<TokenManager<DynProvider>>,
    Arc<UniswapV2Pool<DynProvider>>,
) {
...
async fn test_v2_calculate_tokens_out() {
    let (manager, pool) = setup_v2_pool().await;
    let wbtc = manager.get_token(WBTC_ADDRESS).await.unwrap();
    let amount_in = U256::from(8_000_000_000_u64);
    let expected_amount_out = U256::from_str_radix("847228560678214929944", 10).unwrap();
...
async fn test_v2_calculate_tokens_in() {
    ...
    let amount_out = U256::from_str_radix("1200000000000000000000", 10).unwrap();
...
async fn test_v2_price_calculation() {
    ...
    let expected_nominal = absolute_price * 10_f64.powi(wbtc.decimals() as i32 - weth.decimals() as i32);
    assert!((nominal_price - expected_nominal).abs() < 1e-9, "Nominal price calculation mismatch");

    println!("Nominal price of WBTC/WETH: {}", nominal_price);
    assert!(nominal_price > 10.0 && nominal_price < 20.0, "Nominal price out of expected range");
}
...
async fn test_v2_insufficient_liquidity_swap() {
    ...
    let current_state = pool.get_cached_reserves().await;
    ...
    let result = pool.calculate_tokens_in_from_tokens_out(&weth, weth_reserve).await;

    assert!(
        matches!(result, Err(ArbRsError::CalculationError(ref msg)) if msg.contains("Insufficient liquidity")),
        "Test failed: expected 'Insufficient liquidity' error, got {:?}", result
    );
}
...
fn test_v2_pool_address_generator() {
    ...
    let calculated_address = UniswapV2Pool::<DynProvider>::calculate_pool_address(
        token_a,
        token_b,
        V2_FACTORY_ADDRESS,
        V2_INIT_HASH,
...
async fn test_v2_state_override_calculation() {
    ...
    let expected_override_out = U256::from_str_radix("14947548646999470763", 10).unwrap();
    assert_eq!(override_amount_out, expected_override_out);
}

#[tokio::test]
async fn test_v2_custom_fee() {
    let url = Url::parse(FORK_RPC_URL).expect("Failed to parse RPC URL");
    ...
    let pool_default_fee = UniswapV2Pool::new(WBTC_WETH_POOL_ADDRESS, token0.clone(), token1.clone(), provider_arc.clone(), None);
    let pool_custom_fee = UniswapV2Pool::new(WBTC_WETH_POOL_ADDRESS, token0.clone(), token1.clone(), provider_arc.clone(), Some(25));

    pool_default_fee.update_state().await.unwrap();
    let live_state = pool_default_fee.get_cached_reserves().await;
    let amount_in = U256::from(10_u64.pow(wbtc.decimals() as u32));

    let out_default_fee = pool_default_fee.calculate_tokens_out_with_override(&token0, amount_in, &live_state).unwrap();
    let out_custom_fee = pool_custom_fee.calculate_tokens_out_with_override(&token0, amount_in, &live_state).unwrap();

    assert!(out_custom_fee < out_default_fee);
}
