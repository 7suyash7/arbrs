...
pub const MAX_U160: U256 = U256::from_limbs([0, 0, 0, 1 << 32]);
    ...
    if add {
        ...
        if product / amount == sqrt_price_x_96 {
            ...
            if denominator >= numerator_1 {
                return mul_div_rounding_up(numerator_1, sqrt_price_x_96, denominator).ok_or(
                    ArbRsError::UniswapV3MathError("mul_div_rounding_up failed".into()),
                );
            }
        }
        ...
        if product / amount == sqrt_price_x_96 && numerator_1 > product {
            let denominator = numerator_1 - product;
            mul_div_rounding_up(numerator_1, sqrt_price_x_96, denominator).ok_or(
                ArbRsError::UniswapV3MathError("mul_div_rounding_up failed".into()),
            )
        } else {
...
fn get_next_sqrt_price_from_amount_1_rounding_down(
    ...
    if add {
        ...
        let quotient = mul_div_rounding_up(amount, Q96, liquidity).ok_or(
            ArbRsError::UniswapV3MathError("mul_div_rounding_up failed".into()),
        )?;
...
pub fn get_next_sqrt_price_from_input(
    ...
    if liquidity == 0 {
        return Err(ArbRsError::UniswapV3MathError("L".into()));
    }
    if zero_for_one {
...
pub fn get_next_sqrt_price_from_output(
    ...
    if zero_for_one {
        let quotient = mul_div_rounding_up(amount_out, Q96, liquidity_u256).ok_or(
            ArbRsError::UniswapV3MathError("mul_div_rounding_up failed".into()),
        )?;
        ...
        mul_div_rounding_up(liquidity_u256 << 96, sqrt_p_x96, denominator).ok_or(
            ArbRsError::UniswapV3MathError("mul_div_rounding_up failed".into()),
        )
    }
}
...
pub fn get_amount0_delta(
    ...
    let result = if round_up {
        div_rounding_up(
            mul_div_rounding_up(numerator_1, numerator_2, sqrt_ratio_b_x96)
                .ok_or(ArbRsError::UniswapV3MathError("mul_div failed".into()))?,
            sqrt_ratio_a_x96,
        )
    } else {
        mul_div(numerator_1, numerator_2, sqrt_ratio_b_x96)
            .ok_or(ArbRsError::UniswapV3MathError("mul_div failed".into()))?
            / sqrt_ratio_a_x96
    };
    Ok(result)
...
pub fn get_amount1_delta(
    ...
    if round_up {
        ...
        .ok_or(ArbRsError::UniswapV3MathError(
            "mul_div_rounding_up failed".into(),
        ))
    } else {
        mul_div(
            U256::from(liquidity),
...
pub fn get_amount0_delta_signed(
    ...
    if liquidity < 0 {
        Ok(-I256::from_raw(get_amount0_delta(
            sqrt_ratio_a_x96,
            sqrt_ratio_b_x96,
            (-liquidity) as u128,
            false,
...
pub fn get_amount1_delta_signed(
    ...
    if liquidity < 0 {
        Ok(-I256::from_raw(get_amount1_delta(
            sqrt_ratio_a_x96,
            sqrt_ratio_b_x96,
            (-liquidity) as u128,
            false,
