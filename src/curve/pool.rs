...
use crate::curve::pool_attributes::{PoolAttributes, PoolVariant, SwapStrategyType};
...
// ABI definitions for on-chain calls (unchanged)
sol! {
...
/// The main struct for a Curve Stableswap pool.
pub struct CurveStableswapPool<P: Provider + Send + Sync + 'static + ?Sized> {
    ...
    cached_oracle_rates: RwLock<HashMap<u64, Vec<U256>>>,
}
...
impl<P: Provider + Send + Sync + 'static + ?Sized> LiquidityPool<P> for CurveStableswapPool<P> {
    ...
    /// Fetches the latest on-chain state for the pool (A, fee, balances).
    ...
    /// Calculates the output amount of `token_out` for a given input amount of `token_in`.
    ///
    /// This is the main entry point for swap calculations. It finds the correct
    /// strategy for the pool based on its attributes and delegates the calculation to it.
    async fn calculate_tokens_out(
        ...
        // --- NEW: Smart Dispatcher Logic ---
        let is_metapool = self.base_pool.is_some();
        let is_direct_swap = self.tokens.iter().any(|t| t.address() == token_in.address())
            && self.tokens.iter().any(|t| t.address() == token_out.address());

        if is_metapool && !is_direct_swap {
            return self.calculate_dy_underlying(token_in, token_out, amount_in).await;
        }

        let i = self.tokens.iter().position(|t| t.address() == token_in.address())
            .ok_or_else(|| ArbRsError::CalculationError("Token In not found in pool".to_string()))?;
        let j = self.tokens.iter().position(|t| t.address() == token_out.address())
            .ok_or_else(|| ArbRsError::CalculationError("Token Out not found in pool".to_string()))?;

        let block = self.provider.get_block_by_number(alloy_rpc_types::BlockNumberOrTag::Latest).await?
        ...
        println!("[Swap] Using strategy: {:?}", self.attributes.swap_strategy);
        match self.attributes.swap_strategy {
            SwapStrategyType::Default => DefaultStrategy.calculate_dy(&params).await,
            SwapStrategyType::Metapool => MetapoolStrategy.calculate_dy(&params).await,
            SwapStrategyType::Lending => LendingStrategy.calculate_dy(&params).await,
            SwapStrategyType::Unscaled => UnscaledStrategy.calculate_dy(&params).await,
            SwapStrategyType::DynamicFee => DynamicFeeStrategy.calculate_dy(&params).await,
            SwapStrategyType::Tricrypto => TricryptoStrategy.calculate_dy(&params).await,
            SwapStrategyType::Oracle => OracleStrategy.calculate_dy(&params).await,
            SwapStrategyType::AdminFee => AdminFeeStrategy.calculate_dy(&params).await,
        }
    }
...
impl<P: Provider + Send + Sync + 'static + ?Sized> CurveStableswapPool<P> {
    /// Constructs a new `CurveStableswapPool`.
    /// This function orchestrates the fetching of tokens and the building of pool attributes.
    pub async fn new(
        address: Address,
        provider: Arc<P>,
        ...
        println!("[Pool::new] Creating new pool for address: {}", address);
        ...
        let attributes = attributes_builder::build_attributes(
            address, &tokens, provider.clone(), token_manager, registry
        ).await?;

        let mut base_pool = None;
        if let Some(base_pool_address) = attributes.base_pool_address {
            println!("[Pool::new] Base pool found at {}, recursively creating instance...", base_pool_address);
            let future = Self::new(base_pool_address, provider.clone(), token_manager, registry);
            let bp_instance = Box::pin(future).await?;
        ...
        let pool = Self {
            ...
            provider,
            attributes,
            base_pool, // Store the new pool instance
            a_ramping_state,
        ...
        println!("[Pool::new] Updating initial state for {}...", address);
        pool.update_state().await?;
        println!("[Pool::new] Pool created successfully: {}", address);
        Ok(pool)
    }

    /// Fetches the coin addresses for a pool, robustly handling `int128` and `uint256` ABIs.
    ...
    async fn update_state(&self) -> Result<(), ArbRsError> {
        ...
        *self.fee.write().await = feeCall::abi_decode_returns(&fee_bytes)?;
        ...
        let live_balances = self.fetch_balances().await?; // We'll create this helper

        let final_balances = match self.attributes.swap_strategy {
            SwapStrategyType::AdminFee | SwapStrategyType::DynamicFee | SwapStrategyType::Oracle => {
                // For these strategies, the "true" balance is net of admin fees.
        ...
        *self.balances.write().await = final_balances;
        ...
        // Correctly fetch virtual_price for metapools
        if let Some(base_pool) = &self.base_pool {
            let vp_call = get_virtual_priceCall {};
            let vp_bytes = self.provider.call(TransactionRequest::default().to(base_pool.address).input(vp_call.abi_encode().into())).await?;
            *self.cached_virtual_price.write().await = Some(get_virtual_priceCall::abi_decode_returns(&vp_bytes)?);
    ...
    pub async fn calculate_dy_underlying(
        ...
        if i > 0 && j > 0 {
            ...
            base_pool.calculate_tokens_out(base_token_in, base_token_out, dx).await
            ...
            amounts[base_i] = dx;
            let lp_token_amount = base_pool.calc_token_amount(&amounts, true).await?;
            
            let lp_token = base_pool.lp_token.as_ref();
            let meta_asset_out = token_out;
            self.calculate_tokens_out(lp_token, meta_asset_out, lp_token_amount).await
            ...
            let lp_token = base_pool.lp_token.as_ref();
            let lp_token_amount = self.calculate_tokens_out(meta_asset_in, lp_token, dx).await?;

            let base_j = j - 1;
            let (dy, _fee, _supply) = base_pool.calc_withdraw_one_coin(lp_token_amount, base_j).await?;
            Ok(dy)
    ...
    pub async fn get_scaled_redemption_price(
        ...
        // 1. Check cache first to avoid redundant calls
        if let Some(price) = self.cached_scaled_redemption_price.read().await.get(&block_number) {
            return Ok(*price);
        }

        const REDEMPTION_PRICE_SCALE: u128 = 1_000_000_000; // 10**9
    ...
    pub async fn get_oracle_rates(&self, block_number: u64) -> Result<Vec<U256>, ArbRsError> {
        ...
        let mut rates = Vec::with_capacity(self.attributes.n_coins);
        for i in 0..self.attributes.n_coins {
            let call = price_oracleCall { i: U256::from(i) };
            let bytes = self.provider.call(
                TransactionRequest::default()
                    .to(self.address)
                    .input(call.abi_encode().into())
            ).await?;
            let rate = price_oracleCall::abi_decode_returns(&bytes)?;
            rates.push(rate);
        }
