use crate::{
    balancer::{scaling_helper, weighted_math}, core::token::Token, db::DbManager, errors::ArbRsError, manager::token_manager::TokenManager, pool::{LiquidityPool, PoolSnapshot}, TokenLike
};
...
impl<P: Provider + Send + Sync + 'static + ?Sized> BalancerPool<P> {
    pub async fn new(
        ...
        let pool_tokens_bytes = provider.call(TransactionRequest::default().to(vault).input(IVault::getPoolTokensCall { poolId: pool_id }.abi_encode().into())).await?;
        ...
        let scaling_factors = tokens.iter().map(scaling_helper::compute_scaling_factor).collect();

        Ok(Self { address, provider, tokens, scaling_factors, weights, fee, vault, pool_id: pool_id.0 })
    }
    pub fn fee(&self) -> U256 { self.fee }
}

#[async_trait]
impl<P: Provider + Send + Sync + 'static> LiquidityPool<P> for BalancerPool<P> {
    ...
    async fn get_snapshot(&self, block_number: Option<u64>) -> Result<PoolSnapshot, ArbRsError> {
        ...
        let request = TransactionRequest::default().to(self.vault).input(call.abi_encode().into());
    ...
    fn calculate_tokens_out(
        ...
        let i = self.tokens.iter().position(|t| **t == *token_in).ok_or(ArbRsError::CalculationError("Token In not found".into()))?;
        let j = self.tokens.iter().position(|t| **t == *token_out).ok_or(ArbRsError::CalculationError("Token Out not found".into()))?;
        ...
        for k in 0..self.tokens.len() {
            upscaled_balances[k] = scaling_helper::upscale(upscaled_balances[k], self.scaling_factors[k])?;
        }
        let upscaled_amount_in = scaling_helper::upscale(amount_in_after_fee, self.scaling_factors[i])?;

        let upscaled_amount_out = weighted_math::calc_out_given_in(
            upscaled_balances[i], self.weights[i],
            upscaled_balances[j], self.weights[j],
            upscaled_amount_in,
        )?;
